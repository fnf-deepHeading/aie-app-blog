...
## 디자인 무드 (의도적으로 정한 것)
**깨끗한 오프화이트 종이 + 바이올렛 포인트 + 명조 세리프 헤드라인 + 여백 넉넉한 에디토리얼 레이아웃.** (초기엔 claude.com 톤의 따뜻한 아이보리+테라코타였으나, 2026-06-30 오프화이트+바이올렛 무드로 전환.) 여기에 우리만의 한 끗:

- **시그니처 = 모노스페이스 메타.** eyebrow(카테고리)·날짜·태그·읽는 시간·네비 라벨을 전부 모노스페이스 대문자로 렌더 → "엔지니어링 보이스"를 종이 위에 얹어 App 파트 정체성을 표현.
- **타이포**: 본문 `Pretendard`(한글), 헤드라인 세리프 `Newsreader`(Latin) + `Gowun Batang`(한글), 메타·코드 `JetBrains Mono`. 본문 기본 크기는 17px(`--fs-base`)로 약간 축소된 스케일.
- **베이스 컬러**: 오프화이트 `#FAFAFA`(`--paper`, 바탕) + 흰색 `#FFFFFF`(`--paper-2`, 카드/패널). 잉크·헤어라인은 차가운 그레이 계열.
- **포인트 컬러**: 바이올렛 `#B090F0`(`--clay`, 마스코트에서 추출 — 테두리·불릿·강조 장식)와, 흰 배경에서 가독성 있는 딥 바이올렛 `#7A57D1`(`--clay-2`, 링크·강조 텍스트).
...
- **태그**: 넓은 카테고리형 Title Case로, **전체 고유 태그 15개 이내** 유지(홈 우측 필터). 새 글은 기존 태그 우선 재사용, 정말 새 분류만 추가. 현재 어휘: `Agents`·`Architecture`·`Automation`·`Ops`·`AI Native`·`Tooling`. (15개 초과 시 빌드 경고 — `index.astro`.)
