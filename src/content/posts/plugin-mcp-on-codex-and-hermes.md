...
규칙 이식은 심링크로 걸되, 플러그인 캐시 경로에 **버전이 박혀 있다는**(`.../1.2.18/`) 함정이 있었다. 다음 업데이트에 그 디렉토리가 GC로 사라지면 심링크가 끊긴다. 그래서 현재 버전 경로를 `installed_plugins.json`에서 해소해 재링크하는 걸 월간 cron에 얹었다.
...
4. **버전 박힌 캐시 경로를 심링크하지 마라** — 업데이트 GC에 끊긴다. 현재 경로를 해소해 재링크하는 걸 자동화한다.
