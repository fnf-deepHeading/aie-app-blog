...
| 인증 (결말) | 자동 (플러그인) | DCR 거부 → **매니저 키 Bearer 래퍼** | OAuth SDK hang → **매니저 키 헤더** |
...
**그런데 여기서 한 발 더 갔다.** Codex에서 확인한 그 매니저 장수명 키(`DCSAI_API_KEY`)가 MCP 엔드포인트에서도 먹힌다는 걸 알고, Hermes도 OAuth 대신 config의 `headers`로 그 키를 그냥 꽂아봤다.

```yaml
# ~/.hermes/config.yaml — auth: oauth 를 빼고 headers 로 대체
mcp_servers:
  dcsai:
    url: https://dcsai.fnf.co.kr/server/mcp
    headers:
      Authorization: "Bearer ${DCSAI_API_KEY}"   # .env 에서 런타임 치환(git엔 secret 없음)
    enabled: true
```

`auth: oauth`가 없으면 Hermes는 그 버그난 OAuth 흐름을 **아예 안 탄다**. 재기동하니 바로 붙었다 — `Tools discovered: 7`. **60분 만료·수동 refresh·로그인 hang·핸드오프가 전부 사라졌다.** 결국 위에서 고생한 PKCE 우회는 이 사실을 발견하기 전의 과도기였고, 두 러너 모두 *"매니저 키 static Bearer"* 한 방식으로 수렴했다. 서버가 애초에 그 키를 받아주는 걸 더 일찍 의심했어야 했다.
...
최종 working setup은 DCR을 아예 건너뛰고, **토큰을 Bearer 헤더로 직접 주입하는 래퍼**였다.

처음엔 Hermes가 들고 있는 OAuth 토큰을 읽어 썼는데, 이게 하필 이 셋업에서 **제일 약한 credential**이었다 — 버그난 수동 PKCE로 발급 + 60분 만료 + 자동 refresh도 hang. 그러다 한 가지를 확인했다: **dcsai-manager가 발급하는 장수명 API 키(`DCSAI_API_KEY`)가 MCP 엔드포인트에서도 그대로 먹혔다(실측 200).** OAuth 토큰이 아니라 매니저가 관리하는 키라 만료 churn이 없다. 그래서 래퍼를 이걸로 바꿨다.

```bash
# ~/.codex/bin/dcsai-mcp-remote.sh (요지)
: "${DCSAI_API_KEY:?dcsai-manager 로그인 필요}"   # 매니저 관리 장수명 키
exec npx -y mcp-remote https://dcsai.fnf.co.kr/server/mcp \
  --header "Authorization:Bearer ${DCSAI_API_KEY}" --silent
```
...
전제는 dcsai-manager에 로그인돼 `DCSAI_API_KEY`가 셸 환경에 있는 것뿐이다. Codex를 그 셸에서 띄우면 래퍼가 키를 상속한다. Hermes 토큰에는 더 이상 기대지 않는다 — 약한 고리를 뺀 셈이다.

## 남은 것들 — 정직하게

- **인증은 매니저 키 static Bearer로 통일됐다.** 초기엔 Hermes OAuth 토큰이 60분마다 만료되고 자동 refresh도 버그로 hang해서 갱신 cron까지 팠지만, 결국 Hermes·Codex 둘 다 매니저 장수명 키를 헤더로 꽂는 방식으로 수렴하면서 **만료·refresh 이슈 자체가 없어졌다.** 남은 근본 과제는 OAuth SDK 버그 패치이지만, 실사용 경로에서는 더 이상 그 버그를 밟지 않는다.
