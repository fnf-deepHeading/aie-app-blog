...
- **Hermes 쪽 토큰은 ~60분마다 만료된다.** OAuth 방식이라 그렇고, 자동 refresh도 위 버그로 hang한다 → 갱신 스크립트를 주기 cron으로 돌린다. 다만 이건 **Hermes(OAuth) 한정** 문제다. Codex는 매니저 장수명 키로 옮겨 만료 churn이 없어졌다. 그리고 같은 키가 MCP에서 먹힌다는 건, **Hermes도 OAuth 대신 이 키를 static Bearer로 쓰면 앞의 버그·60분 문제를 통째로 우회**할 수 있다는 뜻이다 — 다음 정리 대상이다.
