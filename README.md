...
tags: ["Agents", "Ops"]          # 선택 — 넓은 카테고리형, 전체 15개 이내 (아래 '작성 규칙 6')
...
### 6. 태그는 넓은 카테고리형 · 전체 15개 이내
태그는 홈 우측 필터로 쓰입니다. **잘게 쪼개지 말고 넓은 카테고리**로(❌ `self-improvement`·`config-management` / ✅ `Ops`·`Tooling`), **Title Case**로 답니다. 새 글은 **되도록 기존 태그에서 고르고**, 정말 새 분류가 필요할 때만 추가하되 **전체 고유 태그가 15개를 넘지 않게** 유지합니다(넘으면 비슷한 것끼리 통합). 글당 3개 안팎 권장.

현재 태그(6): `Agents` · `Architecture` · `Automation` · `Ops` · `AI Native` · `Tooling`. (고유 태그가 15개를 넘으면 `npm run build` 가 경고를 출력합니다.)
